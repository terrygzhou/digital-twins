...
3. **No audit row on preflight failure.** The run never started, so `audit_runs` stays clean; a structured log line carries service + remediation (V). Alternative (write a failed audit row) rejected: it would pollute `kb_run_history` with preflight-only errors and blur the "run happened" invariant. **Ruling 2026-08-30 (diff-reviewer P1, parent adjudicated):** spec FR-002 requires only "no successful audit record". The scheduler's pre-existing R-07 backstop (`loop.py`: "a failed fire is audited — never silent") still writes a `failed` row when the tick dies before any audit row starts — including on preflight failure; this is deliberate and codified in `test_failfast_paths.py`. Ruled: accept the `failed` backstop row on the scheduler path (FR-002 and Constitution V observability both hold; the row is `failed`, never `ok`). The no-row rule applies to `run_pipeline` itself and to the CLI/MCP/web paths, which write no row on preflight failure.
