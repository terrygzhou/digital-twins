...
- [x] 1.1 Add `channel_view(config_dir, env)` in `digital_twins/config/`
...
- [x] 1.2 Add `channel_write(config_dir, updates: dict)` in
...
- [x] 2.1 Add `channels` command group with `list` subcommand:
...
- [x] 2.2 Add `channels enable <name> [--max-items N] [--timeout-s N]`
...
- [x] 3.1 Add `GET /api/config/channels` handler (admin-gated via
...
- [x] 3.2 Add `POST /api/config/channels` handler (admin-gated):
...
- [x] 4.1 Add "Channels" tab/panel to the admin HTML page
...
- [x] 5.1 Add `channel_overrides` to the per-user config schema in
...
- [x] 5.2 Scheduler resolution: in the scheduler's per-run source
...
- [x] 6.1 Document the `channels` CLI group in `docs/configuration.md`
      (list, enable, disable, status, add — with examples).
- [x] 6.2 Document `/api/config/channels` GET/POST in
      `docs/configuration.md` (request/response shape, admin-gated).
- [x] 6.3 Document `channel_overrides` in `docs/configuration.md`
      (per-user section).
- [x] 6.4 Sync `.env.example` / `config.example.yml` if any new env
...
- [x] 7.1 Create `.superpowers/sdd/015-channels-config/progress.md`.
- [x] 7.2 Run `openspec validate channels-config` — must pass.
- [x] 7.3 Confirm `tests/integration/test_portability.py` (T006) and
