...
- [x] 1.1 Add `resolve_backends(flags, env, gpu, docker) -> dict` returning
...
- Pre-existing (unrelated): `tests/unit/test_web_app_kb.py` has 55
  failing/erroring tests (`UnboundLocalError` in the kb_points/kb_search
  app handlers) that fail identically at HEAD — NOT introduced by this
  change. Tracked separately; does not gate install-setup-separation.
- T1.1 landed: `digital_twins.setup.resolve_backends()` (pure function)
  + `tests/unit/test_resolve_backends.py` (7 tests, all pass). Tasks
  1.2/1.3 (CLI flags + wiring run_local_stack to the resolved map) and
  section 2 (interactive per-service prompts) are the next slices.
